...
type CommerceSettings={deliveryEnabled?:boolean;pickupEnabled?:boolean;pickupLabel?:string;businessHours?:string}
...
function Header({tenant,facets,favoriteCount=0,orderCount=0,onFavorites,onOrder}:HeaderProps){
  ...
  return <>
    ...
    <header className="top-wrap">
      <div className="top">
        {tenant&&<button className="mobile-leading-menu" onClick={()=>setMobileOpen(true)} aria-label="Abrir menú"><Menu size={21}/></button>}
...
function StoreHeroCarousel({tenant,total}:{tenant:Tenant;total:number}){
  const fallback:Banner[]=[
    {title:tenant.hero_title||`Descubre ${tenant.public_name}`,subtitle:tenant.hero_subtitle||'Explora productos, precios y disponibilidad actualizados desde CUYRA.',ctaLabel:'Comprar ahora'},
...
function CheckoutConfirmation({tenant,items,onClose,onDone}:{tenant:Tenant;items:OrderItem[];onClose:()=>void;onDone:()=>void}){
  const commerce=tenant.commerce_settings_json||{},defaultMode: 'delivery'|'pickup'=commerce.deliveryEnabled===false?'pickup':'delivery',[mode,setMode]=useState<'delivery'|'pickup'>(defaultMode),[loading,setLoading]=useState(false),[error,setError]=useState(''),total=items.reduce((s,x)=>s+x.priceUsd*x.qty,0),count=items.reduce((s,x)=>s+x.qty,0)
  async function go(){if(!items.length||loading)return;setLoading(true);setError('');try{const r=await fetch('/api/catalog',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({action:'checkout',slug:tenant.slug,fulfillment:mode,items})});const x=await r.json();if(!r.ok)throw new Error(x.error||'No pudimos preparar el pedido');const confirmedItems=Array.isArray(x.items)&&x.items.length?x.items:items,url=openWhatsApp(x.phone||tenant.phone,orderText(tenant,confirmedItems,x.requestCode,mode));if(!url)throw new Error('WhatsApp de ventas no configurado');track(tenant.slug,'whatsapp_order',{items:confirmedItems.length,total:Number(x.total??total),source:'confirmation'});onDone();location.href=url}catch(e:any){setError(String(e?.message||e));setLoading(false)}}
  return <div className="checkout-screen" role="dialog" aria-modal="true"><div className="checkout-shell"><button className="checkout-close" onClick={onClose}><X/></button><div className="checkout-check"><Check/></div><span className="section-kicker">CONFIRMACIÓN</span><h1>¡Tu pedido está listo!</h1><p>{items.length} {items.length===1?'línea':'líneas'} · {count} unidades</p><div className="fulfillment-chooser">{commerce.deliveryEnabled!==false&&<button className={mode==='delivery'?'active':''} onClick={()=>setMode('delivery')}><MapPin/><span><b>Delivery</b><small>Se confirma zona y costo por WhatsApp.</small></span></button>}{commerce.pickupEnabled!==false&&<button className={mode==='pickup'?'active':''} onClick={()=>setMode('pickup')}><Store/><span><b>{commerce.pickupLabel||'Retiro en tienda'}</b><small>{commerce.businessHours||'El asesor confirma disponibilidad y horario.'}</small></span></button>}</div><div className="checkout-items">{items.map(x=><div key={`${x.id}-${x.variantId||0}`}><span><b>{x.name}</b><small>{x.variantLabel||`SKU ${x.sku}`} · x{x.qty}</small></span><strong>{money(x.priceUsd*x.qty)}</strong></div>)}</div><div className="checkout-total"><span>Total referencial</span><strong>{money(total)}</strong></div><p className="checkout-note">Al continuar, CUYRA generará un código de solicitud y abrirá WhatsApp con el pedido organizado. El comercio confirma disponibilidad, total final y condiciones.</p>{error&&<div className="checkout-error">{error}</div>}<button className="checkout-whatsapp" disabled={loading} onClick={go}>{loading?<><LoaderCircle className="spin"/> Preparando pedido…</>:<><MessageCircle/> Continuar por WhatsApp</>}</button><button className="checkout-back" onClick={onClose}>Volver al pedido</button></div></div>
}
...
function ProductCard({tenant,product:p,isFavorite,onFavorite,onAdd}:{tenant:Tenant;product:Product;isFavorite:boolean;onFavorite:(p:Product)=>void;onAdd:(p:Product)=>void}){
  const phone=phoneDigits(tenant.phone||''),out=isOut(p),discount=p.compare_at_price_usd&&p.compare_at_price_usd>p.price_usd?Math.round((1-p.price_usd/p.compare_at_price_usd)*100):0
  ...
  return <article className={`product-card ${out?'is-out':''}`}>
    <div className="product-media-wrap">
      ...
      <div className="card-badges">{p.promo_badge?<span className="promo-badge">{p.promo_badge}</span>:discount>0?<span className="promo-badge">-{discount}%</span>:null}{p.featured&&<span className="featured-badge"><Star size={11}/> Destacado</span>}{tenant.show_stock_mode!=='hidden'&&<span className={out?'stock-badge out':'stock-badge'}>{stockLabel(tenant,p)}</span>}</div>
    ...
    <div className="product-copy">
      ...
      <RatingLine product={p}/><div className="prices">{p.compare_at_price_usd&&p.compare_at_price_usd>p.price_usd&&<del>{money(p.compare_at_price_usd)}</del>}<strong>{p.has_price_range?'Desde ':''}{money(p.price_usd)}</strong>{p.price_bs>0&&<span>{p.has_price_range?'Desde ':''}{bs(p.price_bs)}</span>}</div>
...
function Storefront({slug}:{slug:string}){
  ...
  return <div className={`app theme-${tenant?.catalog_theme||'retail'}`} style={{'--accent':accent,'--tenant-accent':accent} as React.CSSProperties}>
    ...
    <main>
      ...
      {tenant&&<section className="mobile-home-shortcuts" aria-label="Accesos rápidos"><button onClick={()=>document.getElementById('mobile-categories')?.scrollIntoView({behavior:'smooth'})}><span><Grid3X3/></span><b>Categorías</b></button><button onClick={()=>document.querySelector('.brand-rail')?.scrollIntoView({behavior:'smooth'})}><span><Tag/></span><b>Marcas</b></button><button onClick={()=>{setPromoOnly(true);setFeaturedOnly(false);document.getElementById('productos')?.scrollIntoView({behavior:'smooth'})}}><span><Tag/></span><b>Ofertas</b></button><button onClick={()=>setOrderOpen(true)}><span><ShoppingBag/></span><b>Pedido</b>{orderCount>0&&<em>{orderCount}</em>}</button></section>}

      {tenant?.show_category_nav!==false&&showHome('categories')&&data?.facets.categories.length?<section className="mobile-category-strip" id="mobile-categories"><div className="mobile-section-head"><b>Categorías</b><button onClick={()=>{setCategory('');document.getElementById('productos')?.scrollIntoView({behavior:'smooth'})}}>Ver todas</button></div><div>{data.facets.categories.slice(0,10).map((x,i)=><button key={x} className={category===x?'active':''} onClick={()=>{setCategory(x);track(slug,'category_view',{category:x,source:'mobile_home'});document.getElementById('productos')?.scrollIntoView({behavior:'smooth'})}}><span>{x.trim().slice(0,1).toUpperCase()||String(i+1)}</span><small>{x}</small></button>)}</div></section>:null}
      ...
      {tenant&&showHome('newest')&&newestHome.length>0&&<section className="home-commerce-section newest"><div className="section-heading-row"><div><span className="section-kicker">RECIÉN ACTUALIZADO</span><h2>Novedades</h2></div><button onClick={()=>{setSort('newest');document.getElementById('productos')?.scrollIntoView({behavior:'smooth'})}}>Ver catálogo <ArrowRight/></button></div><div className="home-product-rail">{newestHome.map(p=><ProductCard key={`new-${p.source_group_id||p.source_product_id}`} tenant={tenant} product={p} isFavorite={favoriteIds.has(p.source_product_id)} onFavorite={toggleFavoriteProduct} onAdd={addProductToOrder}/>)}</div></section>}
  ...
  return <div className={`app product-page theme-${data.tenant.catalog_theme||'retail'}`} style={{'--accent':data.tenant.accent_color||'#1368ff','--tenant-accent':data.tenant.accent_color||'#1368ff'} as React.CSSProperties}>
    ...
    <main>
      ...
      <section className="product-detail v4-detail">
        ...
        <div className="detail-copy"><div className="detail-brand">{p.brand||p.category||'Producto'}</div><h1>{p.name}</h1><div className="detail-identifiers"><span>SKU <b>{selectedSku}</b></span>{p.model&&<span>Modelo <b>{p.model}</b></span>}{variants.length>1&&<span><b>{variants.length}</b> variantes</span>}</div><div className="detail-prices">{p.compare_at_price_usd&&p.compare_at_price_usd>selectedPrice&&<del>{money(p.compare_at_price_usd)}</del>}<strong>{money(selectedPrice)}</strong>{selectedPriceBs>0&&<span>{bs(selectedPriceBs)}</span>}</div><div className={selectedOut?'availability out':'availability'}>{selectedOut?<><X size={16}/> Agotado</>:<><CheckCircle2 size={16}/> Disponible</>}{selectedStock&&<small>{selectedStock}</small>}</div>
    ...
    <div className="mobile-purchase-dock" aria-label="Acciones de compra">
      ...
      {phone&&<button className="mobile-dock-wa" onClick={()=>{const s=snapshot(p,slug,selected);setOrder(old=>{const idx=old.findIndex(y=>y.id===s.id&&(y.variantId||null)===(s.variantId||null));const next=idx>=0?old.map((y,i)=>i===idx?{...y,qty:y.qty+qty}:y):[...old,{...s,qty}];persist('order',slug,next);return next});setCheckoutOpen(true)}}><ShoppingBag/><span>Comprar</span></button>}
    </div>
