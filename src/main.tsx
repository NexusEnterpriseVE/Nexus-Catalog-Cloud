...
function setTenantManifest(slug=''){const link=document.querySelector('link[rel="manifest"]') as HTMLLinkElement|null;if(link)link.href=slug?`/api/manifest?slug=${encodeURIComponent(slug)}`:'/manifest.webmanifest'}
...
function ProductCard({tenant,product:p,isFavorite,onFavorite,onAdd}:{tenant:Tenant;product:Product;isFavorite:boolean;onFavorite:(p:Product)=>void;onAdd:(p:Product)=>void}){
  ...
  return <article className={`product-card ${out?'is-out':''}`}>
    ...
    <div className="product-copy">
      ...
      {(p.variant_count||1)>1&&p.variant_labels?.length?<div className="variant-preview">{p.variant_labels.slice(0,3).map(x=><span key={x}>{x}</span>)}{p.variant_labels.length>3&&<small>+{p.variant_labels.length-3}</small>}</div>:<div className="variant-preview-spacer"/>}
  ...
  return <div className={`app theme-${data.tenant.catalog_theme||'retail'}`} style={{'--accent':data.tenant.accent_color||'#1368ff','--tenant-accent':data.tenant.accent_color||'#1368ff'} as React.CSSProperties}>
    ...
    <main>
      ...
      <section className="product-detail v4-detail">
        <div className="detail-gallery"><div className="detail-media">{selectedImage?<img src={selectedImage} alt={`${p.name}${selected?.label?` · ${selected.label}`:''}`}/>:<PackageSearch/>}<div className="detail-media-badges">{p.featured&&<span className="featured-badge"><Star size={11}/> Destacado</span>}{data.tenant.show_stock_mode!=='hidden'&&<span className={selectedOut?'stock-badge out':'stock-badge'}>{selectedStock}</span>}</div><button className={`detail-favorite ${favoriteIds.has(p.source_product_id)?'active':''}`} onClick={toggleFav}><Heart fill={favoriteIds.has(p.source_product_id)?'currentColor':'none'}/></button></div>{variants.length>1&&<div className="variant-thumbs">{variants.filter(v=>v.image_url).slice(0,8).map(v=><button key={v.source_product_id} className={selected?.source_product_id===v.source_product_id?'active':''} onClick={()=>setSelectedVariantId(v.source_product_id)} title={v.label}>{v.image_url&&<img src={v.image_url} alt={v.label}/>}</button>)}</div>}<div className="media-caption"><RefreshCw size={15}/><span>Inventario sincronizado con <b>CUYRA</b></span></div></div>
