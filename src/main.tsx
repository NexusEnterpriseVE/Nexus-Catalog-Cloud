...
function waProduct(tenant:Tenant,p:Product){
  const phone=phoneDigits(tenant.phone||'')
  if(!phone)return''
  const text=`Hola, estoy interesado en ${p.name}${p.model?` · Modelo ${p.model}`:''} · SKU ${p.sku}. ¿Está disponible?`
    ...
    <main>
      ...
      <section className="product-detail">
        <div className="detail-gallery">
          ...
          <div className="media-caption"><Store size={15}/><span>Producto publicado desde Nexus Enterprise Pro</span></div>
        </div>
        <div className="detail-copy">
          ...
          <div className="purchase-panel">
            <div><b>¿Te interesa este producto?</b><span>Consulta disponibilidad, condiciones y forma de entrega directamente con la tienda.</span></div>
            {phone&&<a className="primary-action" href={waProduct(data.tenant,p)} target="_blank" rel="noreferrer"><MessageCircle/> Comprar / Consultar por WhatsApp</a>}
          </div>
          ...
          <div className="detail-accordions">
            ...
            <details><summary>Información del producto <ChevronDown size={17}/></summary><div className="accordion-body info-grid"><div><span>Categoría</span><b>{p.category||'—'}</b></div><div><span>Subcategoría</span><b>{p.subcategory||'—'}</b></div><div><span>Marca</span><b>{p.brand||'—'}</b></div><div><span>Modelo</span><b>{p.model||'—'}</b></div></div></details>
            <details><summary>Precio y disponibilidad <ChevronDown size={17}/></summary><div className="accordion-body"><p>Los precios y la disponibilidad se sincronizan desde Nexus Enterprise Pro. {data.tenant.rate_bs_per_usd>0?`Referencia ${data.tenant.rate_source}: ${Number(data.tenant.rate_bs_per_usd).toLocaleString('es-VE')} Bs/USD.`:''}</p></div></details>
          </div>
