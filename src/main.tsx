...
function Storefront({slug}:{slug:string}){
  ...
  const[data,setData]=useState<CatalogResult|null>(null),[q,setQ]=useState(()=>urlParams.get('q')||''),[search,setSearch]=useState(()=>urlParams.get('q')||''),
    ...
    [featuredOnly,setFeaturedOnly]=useState(()=>urlParams.get('featured')==='1'),[sort,setSort]=useState(()=>urlParams.get('sort')||'featured'),[page,setPage]=useState(1),
  ...
  useEffect(()=>setPage(1),[search,category,subcategory,brand,availability,minPrice,maxPrice,featuredOnly,sort])
  ...
  useEffect(()=>{
    ...
    if(brand)p.set('brand',brand);if(availability)p.set('availability',availability);if(minPrice)p.set('minPrice',minPrice);if(maxPrice)p.set('maxPrice',maxPrice);if(featuredOnly)p.set('featured','1')
  ...
  },[slug,search,category,subcategory,brand,availability,minPrice,maxPrice,featuredOnly,sort,page])
  ...
  const activeFilters=[category,subcategory,brand,tenant?.show_stock_mode==='hidden'?'':availability,minPrice,maxPrice,featuredOnly?'featured':''].filter(Boolean).length
  ...
  const clearAll=()=>{setCategory('');setSubcategory('');setBrand('');setAvailability('');setMinPrice('');setMaxPrice('');setFeaturedOnly(false);setQ('')}
  ...
  return <div className={`app theme-${tenant?.catalog_theme||'retail'}`} style={{'--accent':accent,'--tenant-accent':accent} as React.CSSProperties}>
    ...
    <main>
      <section className="hero" id="catalog-home">
