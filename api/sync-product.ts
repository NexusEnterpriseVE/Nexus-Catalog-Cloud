...
type ProductPayload = {
  ...
  brand?: string; model?: string; features?: string; featured?: boolean;
...
async function handlePOST(request: Request) {
  try {
    ...
    const row:any = {
      ...
      description:(p.description||'').trim().slice(0,6000),
      category:(p.category||'').trim().slice(0,180),
      subcategory:(p.subcategory||'').trim().slice(0,180),
      brand:(p.brand||'').trim().slice(0,120),
      model:(p.model||'').trim().slice(0,180),
      features:(p.features||'').trim().slice(0,6000),
      featured:!!p.featured,
